export default function FrameSummary({ product, variantId }) {
  ...
  const variant =
    product.variants?.find((v) => String(v.variantId) === String(variantId)) ||
    product.variants?.[0] ||
    {};
  ...
  return (
    <div className="bg-white rounded-lg p-6 h-fit border border-gray-100 shadow-sm">
      ...
      <div className="space-y-1 mb-4">
        <p className="text-[10px] text-gray-400 uppercase tracking-[0.2em] font-medium font-sans">
          Frame
        </p>
        ...
        <p className="text-sm text-gray-500">
          {product.brand} {variant.color ? `· ${variant.color}` : ""}
        </p>
      </div>
