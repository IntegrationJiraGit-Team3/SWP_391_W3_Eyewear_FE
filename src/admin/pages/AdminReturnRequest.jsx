...
function AdminReturnRequest() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      showToast(
        err?.response?.data?.message || "Không tải được danh sách đổi/trả",
        "error",
      );
    } finally {
      setLoading(false);
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <div className="bg-white px-4 py-2 rounded-xl shadow-sm border border-slate-200 text-sm font-semibold flex items-center gap-2">
            ...
            Role:{" "}
            <span className="text-blue-600 uppercase">{role || "Staff"}</span>
          </div>
          <button
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-slate-50/50">
                <th className="text-left px-6 py-4 text-[11px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100">
                  Request
                </th>
                <th className="text-left px-6 py-4 text-[11px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100">
                  Order Context
                ...
                <th className="text-center px-6 py-4 text-[11px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100">
                  Actions
                </th>
              </tr>
            </thead>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className="text-xs font-bold text-slate-700">
                            Order: #{item.orderCode ?? "N/A"}
                          </span>
                          <span className="text-[10px] font-medium text-slate-400 mt-0.5">
                            Item: #{item.orderItemId ?? "N/A"}
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
