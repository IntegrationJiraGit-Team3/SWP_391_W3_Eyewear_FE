...
function AdminReportsDashboard({ type = "overview" }) {
  ...
  const kpiRates = useMemo(() => {
    ...
    if (processedOrdersResolved <= 0) {
      return { completionRate: 0, refundRate: 0, cancelRate: 0 };
    }

    const refundRate = Number(
      ((refundedOrders / processedOrdersResolved) * 100).toFixed(1),
    );
    const completionRate = Number(
      ((completed / processedOrdersResolved) * 100).toFixed(1),
    );
    const cancelRate = Number(
      ((cancelled / processedOrdersResolved) * 100).toFixed(1),
    );

    return { completionRate, refundRate, cancelRate };
  }, [analytics, refundedOrders, processedOrdersResolved]);
  ...
  const overviewCards = useMemo(() => {
    return [
      ...
      },
      {
        ...
        sub: "Refunded / (Completed + Refunded + Cancelled)",
        icon: FiRefreshCcw,
      ...
      },
      {
        ...
        sub: "Completed / (Completed + Refunded + Cancelled)",
        icon: FiTrendingUp,
      ...
      },
      {
        ...
        sub: "Cancelled / (Completed + Refunded + Cancelled)",
        icon: FiXCircle,
        color: "bg-red-100 text-red-600",
      },
    ];
  }, [analytics, grossRevenue, netRevenue, kpiRates]);
  ...
  const frameQuantityRows = useMemo(
    () => analytics?.bestSellingFramesByQuantity || [],
    [analytics],
  );

  const frameRevenueRows = useMemo(
    () => analytics?.bestSellingFramesByRevenue || [],
    [analytics],
  );
  ...
  const totalProductRevenue = useMemo(() => {
    const raw = analytics?.totalProductRevenue;
    if (raw != null) return toNumber(raw);
    return toNumber(analytics?.grossRevenue ?? analytics?.totalRevenue);
  }, [analytics]);

  const totalLensRevenue = useMemo(() => {
    const raw = analytics?.totalLensRevenue;
    if (raw != null) return toNumber(raw);
    if (!lensRevenueRows.length) return 0;

    return lensRevenueRows.reduce((sum, item) => {
  ...
  }, [analytics, lensRevenueRows]);

  const totalFrameRevenue = useMemo(() => {
    const raw = analytics?.totalFrameRevenue;
    if (raw != null) return toNumber(raw);

    return Math.max(totalProductRevenue - totalLensRevenue, 0);
  }, [analytics, totalLensRevenue, totalProductRevenue]);
  ...
  const frameInventoryRows = useMemo(
    () =>
      productInventoryRows.filter((row) => {
        ...
        return type === "FRAME" || type === "ACCESSORY";
      }),
  ...
  const inventorySummary = useMemo(() => {
    ...
    // ✅ Đồng bộ doanh thu: dùng tổng do BE trả về (không cộng từ bảng Top N).
    const frameRevenue = totalFrameRevenue;
    const lensRevenue = totalLensRevenue;
  ...
  const productQuantityOptions = useMemo(
    () => ({
      ...
      xaxis: {
        categories: productQuantityRows.map((item) => item.productName),
      },
      tooltip: {
        y: {
          formatter: (value) =>
  ...
  const productQuantitySeries = useMemo(
    () => [
      {
        name: "Số lượng bán",
        data: productQuantityRows.map((item) => toNumber(item.quantitySold)),
      },
    ],
    [productQuantityRows],
  );
  ...
  const productRevenueOptions = useMemo(
    () => ({
      ...
      xaxis: {
        categories: productRevenueRows.map((item) => item.productName),
        labels: {
  ...
  const productRevenueSeries = useMemo(
    () => [
      {
        name: "Refund",
        data: productRevenueRows.map((item) =>
  ...
  const topCustomers = useMemo(
    () =>
      analytics?.topCustomersBySpending || analytics?.customerInsights || [],
    [analytics],
  );
        ...
        <>
          ...
          <div className="mt-6 grid grid-cols-1 gap-5 xl:grid-cols-2">
            ...
            <DashboardCard
              ...
              description="Tỉ trọng các trạng thái cuối: Complete, Refund, Cancel trong kỳ."
              icon={<FiShoppingBag className="text-emerald-500" size={20} />}
            >
              {loading ? (
                <LoadingBlock />
              ) : (
                <div className="grid grid-cols-1 gap-5 xl:grid-cols-[1.05fr_0.95fr]">
                  <Chart
                    options={orderOutcomeDonutOptions}
                    series={orderOutcomeDonutSeries}
                    type="donut"
                    height={340}
                  />
                  <SimpleTable
                    headers={["Kết quả", "Số lượng", "Tỉ trọng"]}
                    rows={orderOutcomeGroup.map((item) => [
                      item.label,
      ...
      {type === "orders" && (
        <div className="grid grid-cols-1 gap-5 xl:grid-cols-2">
          <DashboardCard
            ...
            description="Biểu đồ trạng thái tính trên Tổng đơn; KPI outcome tính trên (Completed + Refunded + Cancelled) để 3 tỉ lệ cộng lại bằng 100%."
            icon={<FiShoppingBag className="text-blue-500" size={20} />}
          >
            {loading ? (
              <LoadingBlock />
              ...
              <div>
                ...
                <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4">
                  ...
                  <div className="mt-1 text-xs text-slate-500">
                    Phần này đo theo doanh thu (VND), không phải theo số lượng
                    đơn.
                  </div>
                  <div className="mt-3">
                    <SimpleTable
...
function KpiBox({ title, value, sub }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
      ...
      <div className="mt-2 text-xl font-bold leading-snug text-slate-900 wrap-break-word">
        {value}
      </div>
