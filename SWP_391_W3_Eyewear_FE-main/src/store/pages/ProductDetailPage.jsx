...
function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    const defaultSize = firstVariant?.frameSize || "";

    setSelectedColor((prev) => prev || defaultColor);
    setSelectedSize((prev) => prev || defaultSize);
  }, [productData]);
  ...
  const selectedVariant =
    (!selectedColor && !selectedSize
      ? variants[0]
      : variants.find(
          (v) => v.color === selectedColor && v.frameSize === selectedSize,
        )) || null;
  ...
  const product = {
    ...
    price: formatPrice(
      selectedVariant?.price || productData.price || variants?.[0]?.price || 0,
    ),
    priceNum:
      selectedVariant?.price || productData.price || variants?.[0]?.price || 0,
    category: (
    ...
    images: variants.map((v) => v.imageUrl).filter(Boolean).length
    ...
    specs: [
      ...
      {
        label: "Stock",
        value: variants.reduce((sum, v) => sum + (v.stockQuantity || 0), 0),
      },
      {
  ...
  const selectedVariantUI = selectedVariant;
  let stockText = "",
    stockColor = "",
    isOutOfStock = false;
  ...
  const handleAddToCart = async () => {
    ...
    if (!selectedVariant) {
      showToast("This variant is not available");
      return;
    }

    if (productCat === "lens" && !lensOption) {
      showToast("Please select lens option!");
      return;
    }
    ...
    try {
      const isLens = lensOption === "with-lens";
  ...
  return (
    <>
      <style>{`
        ...
        .choice-pill-active { background: #1c1917; color: #fff; border-color: #1c1917; box-shadow: 0 0 0 2px rgba(28,25,23,0.2); transform: scale(1.05); }
        .choice-pill-size-active { background: #059669; color: #fff; border-color: #059669; box-shadow: 0 0 0 2px rgba(5,150,105,0.18); transform: scale(1.05); }
        .choice-pill { border: 1.5px solid #e7e5e4; padding: 6px 16px; border-radius: 99px; font-size:13px; font-weight:500; transition: all .15s; cursor:pointer; background: white; color: #44403c; }
        .choice-pill:hover { border-color: #a8a29e; }
            ...
            >
                  ...
                  {availableColors.map((color) => (
                    <button
                      ...
                      className={`choice-pill ${
                        selectedColor === color ? "choice-pill-active" : ""
                      }`}
                    >
                      {color}
                    </button>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  {availableSizes.map((size) => (
                    <button
                      ...
                      className={`choice-pill ${
                        selectedSize === size ? "choice-pill-size-active" : ""
                      }`}
                    >
                      {size}
                    </button>
              ...
              <div>
                ...
                <div className="flex items-center border border-stone-200 rounded-full w-fit overflow-hidden bg-stone-50">
                  ...
                  <button
                    disabled={maxStock <= 0 || quantity > maxStock - 1}
...
function LensPurchaseOptions({ lensOption, setLensOption }) {
  return (
    <div className="space-y-3">
      <button
        onClick={() =>
          setLensOption(lensOption === "with-lens" ? null : "with-lens")
        }
        className={`w-full flex items-center gap-3 p-4 rounded-2xl border-2 transition-all text-left ${
          lensOption === "with-lens"
      ...
      >
        <div
          className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors ${
            lensOption === "with-lens" ? "bg-emerald-100" : "bg-stone-100"
          }`}
        >
          <FiEye
            size={17}
            className={
              lensOption === "with-lens" ? "text-emerald-600" : "text-stone-500"
            }
          />
        </div>
        <div className="flex-1 text-left">
          <p className="font-semibold text-sm text-stone-900">Add Lens</p>
          <p
            className={`text-[11px] mt-0.5 ${
              lensOption === "with-lens" ? "text-emerald-600" : "text-stone-400"
            }`}
          >
            Select lens option without prescription form
          </p>
        </div>
      </button>
      ...
      <button
        onClick={() =>
          setLensOption(lensOption === "no-lens" ? null : "no-lens")
        }
        className={`w-full flex items-center gap-3 p-4 rounded-2xl border-2 transition-all text-left ${
          lensOption === "no-lens"
      ...
      >
        <div
          className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
            lensOption === "no-lens" ? "bg-white/10" : "bg-stone-100"
          }`}
        >
          <FiCheck
            size={17}
            className={
              lensOption === "no-lens" ? "text-white" : "text-blue-500"
            }
            strokeWidth={2.5}
          />
        </div>
        <div>
          <p
            className={`font-semibold text-sm ${
              lensOption === "no-lens" ? "text-white" : "text-stone-900"
            }`}
          >
            No Lens
          </p>
          <p
            className={`text-[11px] mt-0.5 ${
              lensOption === "no-lens" ? "text-stone-200" : "text-stone-400"
            }`}
          >
            Buy frame only
          </p>
      ...
      {lensOption && (
        <p className="text-xs text-stone-400 flex items-center gap-1.5 pl-1">
          ...
          <span className="font-semibold text-stone-700">
            {lensOption === "with-lens" ? "With Lens" : "No Lens"}
          </span>
        </p>
      )}
    </div>
